...
(function ($, blocks, el, i18n, _ref) {
  ...
  CaxtonBlock({
    ...
    fields: {
      ...
      'Shadow position': {
        type: 'select',
        options: [{
          value: 'calc( -2px + -.05em ) calc( 2px + .03em )',
          label: 'Far Left'
        }, {
          value: 'calc( -1px + -.03em ) calc( 1px + .01em )',
          label: 'Left'
        }, {
          value: '0 0',
          label: 'Center'
  ...
  CaxtonBlock({
    ...
    fields: {
      ...
      'Text Shadow position': {
        type: 'select',
        options: [{
          value: 'calc( -3px + -.07em ) calc( 2px + .03em )',
          label: 'Far Left'
        }, {
          value: 'calc( -1px + -.03em ) calc( 1px + .01em )',
          label: 'Left'
  ...
  CaxtonBlock({
    ...
    fields: {
      ...
      'Gradient type': {
        ...
        options: [{
          ...
          value: 'linear-gradient( 45deg, ',
          label: 'Linear 45 deg'
        }, {
          value: 'linear-gradient( -45deg, ',
          label: 'Linear 45 deg anticlockwise'
        }, {
    ...
    edit: function edit(props, block) {
      ...
      if (block.attrs['Layout']) {
        cls += " ".concat(block.attrs['Layout']);
      }

      if (block.attrs['Column gap']) {
        colCls += " ".concat(block.attrs['Column gap']);
      }
      ...
      el('div', {
        className: "".concat(colCls, " caxton-").concat(columns, "-columns"),
        style: {
          'paddingTop': padT,
          'paddingLeft': padL,
          'paddingBottom': padB,
        ...
        'data-tablet-css': "padding-left:".concat(padTab, "em;padding-right:").concat(padTab, "em;"),
        'data-mobile-css': "padding-left:".concat(padMob, "em;padding-right:").concat(padMob, "em;"),
        key: 'block'
  ...
  var CaxtonPostsGridEditComponent =
  /*#__PURE__*/
  function (_React$Component) {
    ...
    function CaxtonPostsGridEditComponent(props) {
      ...
      _this = _possibleConstructorReturn(this, _getPrototypeOf(CaxtonPostsGridEditComponent).apply(this, arguments));
      _this.state = {
    ...
    _createClass(CaxtonPostsGridEditComponent, [{
      ...
      value: function fetchUrls() {
        ...
        var url = "/caxton/v1/posts?posts_per_page=".concat(attrs.rows * attrs.columns, "&post__not_in=").concat(caxton.post, "&cat=").concat(attrs.cat).concat(attrs.postIDs ? "&post__in=".concat(attrs.postIDs) : '').concat(attrs.displayPostWithoutImages ? '&meta_key=' : '', "&orderby=").concat(order[0], "&order=").concat(order[1]);
      ...
      value: function render() {
        ...
        var className = "".concat(props.className, " ").concat(props.name.replace('/', '-'), " caxton-grid");
        className += " caxton-".concat(attrs.imagesType, "-images");
          ...
          for (var i = 0; i < props.posts.data.length; i++) {
            var postMetaMarkup = function postMetaMarkup() {
              if (attrs.displayMeta) {
                return {
                  __html: "<span class=\"author\">".concat(authorIcon).concat(post.author, "</span><span class=\"comments\">").concat(commentIcon, " ").concat(post.comments, "</span>")
                };
              } else {
                return {
                  __html: ''
            ...
            gridInfo = [el('a', {
              ...
              style: {
                backgroundImage: "url(".concat(post.thumb_ml, ")")
              }
            }, el('h3', {
              className: 'grid-title',
              style: {
                fontSize: attrs.titleSize
              }
            }, post.title)))];
            gridInfo.push(el('h3', {
              className: 'grid-title',
              ...
              key: 'title'
            }, $("<div/>").html(post.title).text()));

            if (attrs.displayDate) {
              gridInfo.push(el('time', {
                key: 'date'
              }, post.date));
            }
            ...
            grids.push(el('div', {
              className: postClass,
              style: {
                width: "".concat(100 / attrs.columns - 2, "%")
              },
        ...
        return [!!selected && el(InspectorControls, {
          key: 'inspector'
        }, el(SelectControl, {
          label: 'Category',
          value: attrs.cat,
          ...
          onChange: function onChange(val) {
            if (val.target) {
              props.setAttributes({
                displayPostWithoutImages: val.target.checked
              });
            } else {
              props.setAttributes({
                displayPostWithoutImages: val
              });
            }
          }
        }), el(ToggleControl, {
          ...
          onChange: function onChange(val) {
            if (val.target) {
              props.setAttributes({
                displayDate: val.target.checked
              });
            } else {
              props.setAttributes({
                displayDate: val
          ...
          onChange: function onChange(val) {
            if (val.target) {
              props.setAttributes({
                displayMeta: val.target.checked
              });
            } else {
              props.setAttributes({
                displayMeta: val
          ...
          onChange: function onChange(val) {
            if (val.target) {
              props.setAttributes({
                displayExcerpt: val.target.checked
              });
            } else {
              props.setAttributes({
                displayExcerpt: val
          ...
          onChange: function onChange(val) {
            if (val.target) {
              props.setAttributes({
                titleBelowImage: val.target.checked
              });
            } else {
              props.setAttributes({
                titleBelowImage: val
...
exports.sectionFields = exports.gridFields = void 0;
var gridFields = {
  'Columns': {
    type: 'range',
    min: 1,
...
/*#__PURE__*/
function (_React$Component) {
  _inherits(GridRender, _React$Component);

  function GridRender(props) {
    _classCallCheck(this, GridRender);

    return _possibleConstructorReturn(this, _getPrototypeOf(GridRender).call(this, props));
  }
...
exports.sectionRender = void 0;

var sectionRender = function sectionRender(props, block, childrenBlocks) {
