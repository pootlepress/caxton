function initCaxton( $, blocks, el, i18n, components ) {
	...
	CxB.prototype.colorFieldInit = function( field ) {
		var panelChildren = [], props = this.fieldProps( field );
	...
	CxB.prototype.iconFieldInit = function( field ) {
		var props = this.fieldProps( field ), that = this, searchTerms = [];
		props.title = props.label;
	...
	CxB.prototype.inspectorFields = function () {
		...
		for ( var id in this.sections ) {
			if ( this.sections.hasOwnProperty( id ) ) {
				panelFields = this.renderFields( this.sections[ id ], id )
				els.push(
					el(
						components.PanelBody,
						{
...
//				html = html.split( '[' + fld.id + ']' ).join( val );
	...
	CxB.prototype.registerBlock = function () {
		...
		registerBlockProps.getEditWrapperProps = function( attributes ) {
			var fullWidth = attributes['Full width'];

			if ( fullWidth ) {
				return { 'caxton-layout': fullWidth };
			}

			if ( typeof block.registerBlockProps === 'function' ) {
				return block.registerBlockProps( attributes, that );
			}
		};
