function initCaxton( $, blocks, el, i18n, components ) {
	...
	CxB.prototype.fieldProps = function( field, index ) {
		var
			id = field.id,
			that = this,
	...
	CxB.prototype.imageFieldInit = function( field, index ) {
		var props = this.fieldProps( field, index );
		if ( ! props.className ) {
			props.className = '';
		}

		var btnContent = __( 'Select image' ), removeBtn = null;
	...
	CxB.prototype.colorFieldInit = function( field, index ) {
	...
	CxB.prototype.radioFieldInit = function( field, index ) {
		var fieldProps = this.fieldProps( field, index );
		fieldProps.selected = fieldProps.value;
		return el( components.RadioControl, fieldProps );
	};
	CxB.prototype.rangeFieldInit = function( field, index ) {
		return el(
			components.RangeControl,
			this.fieldProps( field, index )
		)
	};
	CxB.prototype.selectFieldInit = function( field, index ) {
		return el(
	...
	CxB.prototype.textFieldInit = function( field, index ) {
		return el(
			components.TextControl,
			this.fieldProps( field, index )
		)
	};
	CxB.prototype.textareaFieldInit = function( field, index ) {
		return el(
			components.TextareaControl,
			this.fieldProps( field, index )
		)
	};
	CxB.prototype.toggleFieldInit = function( field, index ) {
		var fieldProps = this.fieldProps( field, index );
		fieldProps.checked = !! this.attrs[ field.id ];
...
//			{value: 'right bottom', label: 'Right bottom',},
	...
	CxB.prototype.toolbarElements = function () {
	...
	CxB.prototype.registerBlock = function () {
		var
			...
			that = this,
			block = this.block,
			registerBlockProps = $.extend( {}, block );
		if ( block.icon.indexOf( '<svg' ) > - 1 ) {
			var $icon = jQuery( block.icon );
			var props = {};
			$.each( $icon[0].attributes, function () {
				if ( this.specified ) {
					attrName = this.name.replace(/[-:]([a-z])/g, function (g) { return g[1].toUpperCase(); });
